% GLM simulation 
function [spiketime, simSpiketrain] = simNeuCpl_batch(resp, dt, rep, nlfun, ih)
[rlen,batch,ncells] = size(resp);
dc = max(reshape(resp,[],N))*0.85;
resp = bsxfun(@minus,resp,dc);

simSpiketrain = zeros(rep, rlen, batch, ncells);
clear spiketime;
nbinsPerEval = 100;
hlen = size(ih,1);
% hlen = 0;
